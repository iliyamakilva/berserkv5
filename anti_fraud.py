"""
سیستم ضد تقلب سبک‌وزن.

⚠️ محدودیت مهم: Telegram Bot API هیچ دسترسی به IP یا فینگرپرینت دستگاه
نمی‌ده، پس تشخیص واقعی "ساخت چند اکانت" از طریق ربات ممکن نیست.
چیزی که این ماژول واقعاً چک می‌کنه الگوهای قابل مشاهده از دیتای رباته:
خود-معرفی (self-referral)، اکانت‌های بن‌شده، و سقف معقول برای تعداد
پاداش رفرال (کلی و روزانه) تا از فارمینگ جلوگیری بشه.
"""
...
def is_referral_suspicious(ref_id, referred_id):
    """برمی‌گردونه (suspicious: bool, reason: str|None)."""
...
def flag_for_review(reason, ref_id, referred_id):
    """پیام آماده برای اطلاع‌رسانی به ادمین وقتی پاداشی بلاک میشه."""
