...
v5.1 UX hotfix:
- دکمه بازگشت به پنل مدیریت به خروجی‌های اصلی اضافه شد.
- منوی مدیریت با منوی ثابت پایین کار می‌کند و کاربر/ادمین مجبور به /start نیست.
"""
...
async def process_search(m: types.Message, state: FSMContext):
    ...
    for r in rows[:10]:
        ...
        if owned:
            text += "\n\n📦 اکانت‌های این کاربر:\n" + "\n".join(
                f"• {s['account_name'] or '-'} ({s['assigned_at']})"
                for s in owned
            )
...
async def process_balance_id(m: types.Message, state: FSMContext):
    ...
    if not db.get_user(target):
        return await m.answer("این کاربر پیدا نشد.\nدوباره بفرستید یا لغو کنید:", reply_markup=cancel_kb())
...
async def cb_msgkey(c: types.CallbackQuery, state: FSMContext):
    ...
    await c.message.answer(
        ...
        "• یه عکس بفرستید → عکس عوض میشه\n"
        "• دستور /clear رو بفرستید → بنر پاک میشه",
        reply_markup=cancel_kb(),
    )
...
async def process_message_edit(m: types.Message, state: FSMContext):
    ...
    if m.content_type == "text":
        if m.text.strip() == "/clear":
            messages.clear(key)
            await state.finish()
            return await m.answer("✅ بنر این صفحه پاک شد و به حالت پیش‌فرض برگشت.", reply_markup=messages_menu_kb())
    ...
    await m.answer("لطفا فقط متن یا عکس بفرستید (یا /clear برای پاک کردن).", reply_markup=cancel_kb())
...
async def cb_restore_start(c: types.CallbackQuery):
    ...
    await c.message.answer(
        ...
        "قبل از جایگزینی، نسخه امن از دیتابیس فعلی نگه داشته میشه.",
        reply_markup=cancel_kb(),
    )
...
async def process_restore_file(m: types.Message, state: FSMContext):
    ...
    if not backup.validate_sqlite_file(tmp_path):
        os.remove(tmp_path)
        return await m.answer("❌ این فایل دیتابیس معتبر نیست. بازگردانی لغو شد.", reply_markup=admin_back_kb())
