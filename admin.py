...
async def cb_users(c: types.CallbackQuery):
    ...
    for index, r in enumerate(rows, start=1):
        ...
        referral_count = int(r["referral_count"] or 0) if "referral_count" in r.keys() else 0
        referral_test_count = int(r["referral_test_count"] or 0) if "referral_test_count" in r.keys() else 0
        referral_suffix = f" | رفرال: {referral_count}"
        if referral_test_count:
            referral_suffix += f" + {referral_test_count} تست"
        lines.append(
            f"{index}. {flag} {r['id']} | {username}{test_mark} | خرید: {r['purchased']} | سرویس: {delivered}{referral_suffix} | موجودی: {_fmt_money(r['balance'])} | عضویت: {_dual(r['joined_at'])}"
        )
