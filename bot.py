...
async def check_low_stock_alert():
    """
    بعد از هر فروش صدا زده میشه.
    اگر موجودی به آستانه رسید، فقط یکبار تا شارژ مجدد به ادمین هشدار می‌دهد.
    """
...
async def buy_qty(c: types.CallbackQuery):
    ...
    if not purchased_items:
        db.add_balance(user_id, total_price)
        return await c.message.answer("❌ خرید انجام نشد؛ موجودی لینک تمام شد.", reply_markup=menus.main_reply_kb(c.from_user.id))

    if len(purchased_items) != qty:
        # در حالت نادر، اگر وسط کار موجودی تمام شد، مبلغ لینک‌های تحویل‌نشده را برمی‌گردانیم.
...
async def confirm_buy(c: types.CallbackQuery):
    """
    سازگاری با نسخه قبل:
    اگر دکمه قدیمی confirm_buy جایی باقی مانده باشد، آن را به خرید یک‌عددی وصل می‌کنیم.
    """
    # در نسخه جدید، خرید تک‌عددی همان مسیر buy_qty_1 است.
    class _Shim:
        data = "buy_qty_1"
